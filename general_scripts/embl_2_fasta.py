...
def process_file(args):
    ...
    with open_embl(embl_path) as handle, open(out_path, "w") as out_fh:
        contig_idx = 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert EMBL / EMBL.GZ files to FASTA (.fa) using Biopython (parallel)"
    )
    parser.add_argument(
        "inputs",
        nargs="+",
        help="Input files or glob patterns (e.g. *.embl qwe/asd/*.embl)"
    )
    parser.add_argument(
        "-o", "--outdir",
        default=".",
