...
def process_file(args):
    ...
    if not proteins:
        return f"[WARN] {embl_path}: no CDS with locus_tag + translation found"

    with open(out_path, "w") as out_fh:
        SeqIO.write(proteins, out_fh, "fasta")

    return f"[OK] {embl_path} → {out_path} ({len(proteins)} proteins)"
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert EMBL / EMBL.GZ files to FAA using locus_tag (parallel)"
    )
    parser.add_argument(
        "inputs",
        nargs="+",
