...
def process_file(args):
    ...
    out_path = os.path.join(outdir, f"{name}.gff")
    ...
    with open(out_path, "w") as out_fh:
        GFF.write(records, out_fh)  # writes in GFF3-like format but file extension .gff
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert GBK / GB files to GFF (.gff) using Biopython/BCBio (parallel)"
    )
    parser.add_argument(
        "inputs",
        nargs="+",
